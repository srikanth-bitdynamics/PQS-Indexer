...
object ConvertEventSpec extends ZIOSpecDefault:
  ...
  private val template: Template[Descriptor] =
    Template(
  ...
  private given ProtobufCodecs = Dictionary(Seq.empty)

  private given DamlSchema =
    new DamlSchema(
      schema = Dictionary(Seq(template)),
  ...
  def spec = suite("convertEvent")(
    test("retains witness parties on an archived event"):
      ...
      for converted <- specific.convertEvent(Event(Event.Event.Archived(archived)))
