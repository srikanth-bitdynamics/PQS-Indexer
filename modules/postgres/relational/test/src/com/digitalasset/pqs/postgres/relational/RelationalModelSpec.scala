...
object RelationalModelSpec extends ZIOSpecDefault:
  ...
  def spec = suite("relational model")(
    test("COPY headers carry the /*N*/ order, table names and columns matching V001"):
      ...
      assertTrue(
        headers(0) ==
          "/*0*/ copy __rel_transactions (tx_ix, ledger_offset, transaction_id, effective_at, record_time, synchronizer_id, workflow_id, external_transaction_hash, paid_traffic_cost) from stdin",
        headers(1) ==
        ...
        headers(3) ==
          "/*3*/ copy __rel_contracts (contract_pk, contract_id, template_entity_pk, representative_package_id, creation_package_id, created_tx_ix, created_at_offset, signatories, observers, create_witnesses, synchronizer_id, metadata, contract_key_json, contract_key_hash, divulged_only, source_kind, history_lower_bound) from stdin",
        headers(4) ==
    ...
    test("payload and interface-view rows target the resolved dynamic table name"):
      val f = factory
      val payload =
        model.ContractPayload(specific.ContractPayload(f.mk, ujson.Str("x"), None), "rel_asset__asset__asset")
      val view = model.InterfaceView(specific.InterfaceView(f.mk, ujson.Str("v")), "relv_iface__iface__iface")
      assertTrue(
        payload._sql == "/*7*/ copy rel_asset__asset__asset (contract_pk, payload_json, metadata) from stdin",
        payload._row == "1\t\"x\"\t\\N",
