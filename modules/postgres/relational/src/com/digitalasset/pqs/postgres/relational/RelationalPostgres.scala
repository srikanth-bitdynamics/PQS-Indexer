...
import com.digitalasset.canonical.{ContractId, Party, ReassignmentEvent, UserRight}
      ...
      case c: canonical.specific.Event.Created =>
        ...
        c.payloads.find { (id, _) => isTemplate(id) }.fold(Chunk.empty[model.Model]) { (templateId, templateDv) =>
          ...
          val payload = model.ContractPayload(
            specific.ContractPayload(contractPk, codec.template(templateId).fromDynamicValue(templateDv), c.metadata),
    ...
    traces.root("process metadata and schema") {
      for
        ...
        exercises <- transaction {
          sql"""select pkg.id, c.module_name, c.entity_name, c.choice, c.pk
                from __rel_choice c join __rel_package pkg on c.package_name = pkg.name"""
